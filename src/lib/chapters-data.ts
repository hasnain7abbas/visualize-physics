    ...
    sections: [
      {
        ...
        definition:
          "When a wave source moves through the medium at speed $v_s$ while emitting at rest-frame frequency $f$, the wavefronts pile up ahead of it and spread out behind — the **Doppler effect**. An observer detects a Doppler-shifted frequency $f' = f\\,v_w/(v_w - v_s\\cos\\theta)$, where $v_w$ is the wave speed and $\\theta$ is the angle between the source's velocity and the line to the observer. Approaching ($\\theta = 0$) gives a higher frequency (blueshift), receding ($\\theta = \\pi$) gives a lower one (redshift) — the everyday effect of a passing siren and the principle behind radar speed guns.",
        statisticalTools: [
...
const solidState: Chapter[] = [
  {
    ...
    color: "#6366f1",
    icon: "\u{1F48E}",
    shortDesc: "Crystals & bands",
    sections: [
      {
        id: "crystal-lattice",
        title: "Crystal & Reciprocal Lattices",
        description:
          "The five 2D Bravais lattices and their reciprocal counterparts. Primitive vectors $\\vec a_1, \\vec a_2$ and dual $\\vec b_1, \\vec b_2$ satisfying $\\vec a_i \\cdot \\vec b_j = 2\\pi\\delta_{ij}$.",
        definition:
          "A **Bravais lattice** is an infinite set of points generated by integer combinations of primitive vectors: $\\vec R = n_1 \\vec a_1 + n_2 \\vec a_2$ (in 2D; add $\\vec a_3$ in 3D). In two dimensions there are exactly five such lattices — square, triangular, rectangular, oblique, and centered rectangular — and every 2D crystal is built by placing a basis of atoms at each lattice point. The **reciprocal lattice** is defined by the dual vectors $\\vec b_i$ satisfying $\\vec a_i \\cdot \\vec b_j = 2\\pi \\delta_{ij}$; it is the natural setting for diffraction (Bragg condition: scattering vector = reciprocal lattice vector) and for Bloch's theorem in the band theory of solids.",
        statisticalTools: [
          { name: "Bravais Lattice", desc: "The set $\\{n_1 \\vec a_1 + n_2 \\vec a_2 : n_i \\in \\mathbb Z\\}$. Looks the same from any lattice point. Classification by point symmetry gives 5 in 2D, 14 in 3D." },
          { name: "Primitive Unit Cell", desc: "A region containing exactly one lattice point; tiles space by translation. Not unique — the parallelogram spanned by $\\vec a_1, \\vec a_2$ is the standard choice." },
          { name: "Wigner-Seitz Cell", desc: "The set of points closer to a given lattice point than to any other. For the square lattice a square; for the triangular lattice a hexagon. The Wigner-Seitz cell of the reciprocal lattice = first Brillouin zone." },
          { name: "Reciprocal Lattice", desc: "$\\vec G = m_1 \\vec b_1 + m_2 \\vec b_2$ with $\\vec a_i \\cdot \\vec b_j = 2\\pi\\delta_{ij}$. Dual of the direct lattice — Fourier components of any function periodic in the direct lattice are indexed by $\\vec G$." },
          { name: "Brillouin Zone", desc: "The Wigner-Seitz cell of the reciprocal lattice. All distinct crystal momenta live here; adding $\\vec G$ gives an equivalent state. Hexagonal for a triangular real-space lattice (graphene, silicon (111))." },
          { name: "Miller Indices (h,k,l)", desc: "Label a family of parallel lattice planes perpendicular to $\\vec G = h\\vec b_1 + k\\vec b_2 + l\\vec b_3$. Plane spacing $d_{hkl} = 2\\pi/|\\vec G|$." },
          { name: "Basis & Structure Factor", desc: "$F_{\\vec G} = \\sum_j f_j e^{i\\vec G \\cdot \\vec r_j}$ sums over atoms in the basis. Determines which Bragg peaks are bright/dark (selection rules for bcc, fcc)." },
          { name: "Laue/Bragg Condition", desc: "$\\vec k_{\\text{out}} - \\vec k_{\\text{in}} = \\vec G$, equivalent to $2d\\sin\\theta = n\\lambda$. The basis of X-ray and electron diffraction." },
          { name: "Ewald Sphere", desc: "Geometric construction: diffraction occurs when a reciprocal lattice point lies on a sphere of radius $|\\vec k|$ in reciprocal space. Rotating the crystal sweeps new points onto the sphere." },
          { name: "Point Groups & Space Groups", desc: "2D Bravais lattices × basis symmetries = 17 wallpaper groups. In 3D: 32 point groups × 14 Bravais = 230 space groups. The full classification of crystalline symmetry." },
        ],
        keyEquations: [
          "\\vec R = n_1 \\vec a_1 + n_2 \\vec a_2,\\quad n_i \\in \\mathbb Z",
          "\\vec a_i \\cdot \\vec b_j = 2\\pi\\,\\delta_{ij}",
          "2d\\sin\\theta = n\\lambda",
        ],
        conceptSummary:
          "Switch between the five 2D Bravais lattices and see both the direct and reciprocal pictures. The triangular direct lattice maps to a rotated hexagonal reciprocal — the building block of the Brillouin zone of graphene.",
      },
      {
        id: "band-structure",
        title: "Band Structure (Nearly-Free Electrons)",
        description:
          "A weak periodic potential opens gaps in the free-electron dispersion at Brillouin zone boundaries — the origin of the metal-insulator-semiconductor distinction.",
        definition:
          "In the **nearly-free electron** model, conduction electrons are plane waves $\\psi_k = e^{ikx}$ perturbed by a weak periodic crystal potential $V(x) = V(x+a)$. **Bloch's theorem** says the exact eigenstates have the form $\\psi_{nk}(x) = e^{ikx} u_{nk}(x)$ with $u_{nk}$ periodic, and the energy $E_n(k)$ is a multi-valued function of $k$ organized into **bands**. At zone boundaries $k = \\pm\\pi/a$ the potential mixes plane waves differing by a reciprocal lattice vector $G = 2\\pi/a$, lifting the degeneracy and opening a **band gap** $2|V_G|$. Whether the highest filled band overlaps the next empty one determines metal vs. insulator vs. semiconductor.",
        statisticalTools: [
          { name: "Bloch's Theorem", desc: "$\\psi_{nk}(\\vec r) = e^{i\\vec k\\cdot\\vec r} u_{nk}(\\vec r)$ with $u$ periodic. $\\vec k$ lives in the Brillouin zone; $n$ labels bands. The foundation of all electronic structure in solids." },
          { name: "Reduced Zone Scheme", desc: "All bands plotted within the first Brillouin zone by folding higher-$k$ branches via $k \\to k - G$. Gives a compact and unambiguous picture of the band structure." },
          { name: "Band Gap", desc: "Energy range with no allowed states. At a zone boundary, degenerate perturbation theory gives gap $= 2|V_G|$ to first order." },
          { name: "Central Equation", desc: "In plane-wave basis: $(\\hbar^2(\\vec k+\\vec G)^2/2m - E)\\,c_{\\vec G} + \\sum_{\\vec G'} V_{\\vec G - \\vec G'} c_{\\vec G'} = 0$. A matrix eigenvalue problem at each $\\vec k$." },
          { name: "Effective Mass", desc: "$m^* = \\hbar^2 / (d^2E/dk^2)^{-1}$. Near a band extremum, electrons behave as free particles with a renormalized mass — positive near a minimum (conduction band), negative near a maximum (gives rise to holes)." },
          { name: "Fermi Level", desc: "Highest filled energy at $T=0$. Its location relative to gaps distinguishes metal (inside a band), semiconductor (in a narrow gap), and insulator (in a wide gap)." },
          { name: "Density of States D(E)", desc: "Number of states per unit energy. Has Van Hove singularities at band edges and saddle points — visible as peaks and kinks in optical absorption spectra." },
          { name: "Tight-Binding Model", desc: "Complementary picture starting from atomic orbitals. Hopping $t$ between neighbors gives bands of width $\\sim 4t$ (in 2D square). Good near the strong-potential limit." },
          { name: "Fermi Surface", desc: "Constant-energy surface at $E_F$ in $\\vec k$-space. Spherical for free electrons; distorted near zone boundaries. Topology of the Fermi surface controls transport." },
          { name: "Electronic Phases", desc: "Filled band = no conductivity; half-filled band = metal. Small gap = semiconductor (thermally excited carriers); large gap = insulator. Direct vs indirect gap determines optical properties." },
        ],
        keyEquations: [
          "E(k) = \\frac{\\hbar^2 k^2}{2m}\\quad \\text{(free electron)}",
          "\\psi_{nk}(\\vec r) = e^{i\\vec k\\cdot\\vec r}\\, u_{nk}(\\vec r) \\quad \\text{(Bloch)}",
          "\\Delta E_{\\text{gap}} = 2|V_G|",
        ],
        conceptSummary:
          "Start with $V_0 = 0$ — the bands are the free-electron parabolas folded into the first Brillouin zone. Crank up $V_0$ and watch gaps open at $k = \\pm\\pi/a$ and at $k=0$. At large $V_0$ the bands flatten, approaching the atomic-orbital (tight-binding) limit.",
      },
    ],
  },
  {
...
export const chapterGroups: ChapterGroup[] = [
  ...
  },
  {
    id: "solid-state",
    title: "Solid State Physics",
    subtitle: "Crystal lattices & band structure",
    chapters: solidState,
  },
];
