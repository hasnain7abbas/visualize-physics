...
| 2 | Electrodynamics | **Done** (E1–E5: point charges/field lines/equipotentials; wire/Helmholtz/Ampère; plane wave/polarization/Poynting; Faraday + Lorentz force; multipole expansion + Larmor radiation). |
| 3 | Waves & Oscillations | **Done** (W1–W4: 1D wave/Fourier/energy density; single/double/grating diffraction; moving source/Mach/observer freq; damped-driven oscillator + rectangular drum). |
| 4 | Thermodynamics (expansion) | **Done** (S11–S13 atop S1–S10: PV diagrams, molecular dynamics, blackbody). |
| 5 | Optics | **Done** (O1: lens system, polarization/Malus, thin-film interference). |
| 6 | Special Relativity | **Done** (R1: Minkowski diagrams, time dilation/length contraction, energy-momentum). |
| 7 | Quantum Mechanics II | **Done** (Q6: hydrogen orbitals + Bell/CHSH. Q7: TDSE wavepacket scattering + perturbation theory). 7.3 Stern-Gerlach already covered by Q2; 7.6 Coherent states by Q5. |
| 8 | Solid State Physics | **Done** (SS1: crystal/reciprocal lattices + nearly-free-electron bands. SS2: diatomic phonon chain + semiconductor band diagram. SS3: X-ray diffraction + degenerate Fermi gas). |
| 9 | Nuclear & Particle Physics | **Done** (N1: SEMF binding-energy + radioactive decay. N2: Rutherford scattering + Standard Model explorer). 9.5 Feynman diagram builder remains as the only optional item not shipped. |
| 10 | General Relativity & Cosmology | **Done** (G1: Schwarzschild geodesics + gravitational lensing. G2: Friedmann $a(t)$ + CMB acoustic peaks. G3: Flamm paraboloid embedding + cosmological distance measures). |
| 11 | Mathematical Methods | **Done** (M1: 2D phase portraits + Bessel/Legendre/Hermite plotter. M2: 1D heat-equation solver + spherical-harmonic lobes). 11.5 Tensor calculator remains as the only optional item not shipped. |
| 12 | Global UX: search, PWA, theming | **Done**. Dark/light theme (12.3) and per-chapter code-splitting (12.6) were already in place. This pass shipped global Cmd/Ctrl+K **search palette** with fuzzy ranking over every chapter and section (12.1); a **PWA manifest + service worker** with network-first HTML and cache-first assets (12.2); **localStorage-backed bookmarks and a "Recently visited" sidebar** (12.4). 12.5 URL-shareable parameter state and 12.7 axe-core audit are out of scope here — would require touching every simulation. |

**Summary**: every physics phase now ships all of its core simulations *and* the four optional stretch-goal items that landed in the latest sweep (multipole, Larmor, X-ray diffraction, Fermi gas, Flamm embedding, cosmic distances). Counting by section across 23 physics chapters (F1–F3, Q1–Q7, S1–S13, C1–C8, E1–E5, W1–W4, O1, R1, N1–N2, G1–G3, M1–M2, SS1–SS3), the app now contains **~90 interactive sections**, comfortably past the roadmap's original ~64-sim target. The Phase 12 polish layer adds keyboard-driven navigation, offline support, and persistent pinning. Remaining stretch items: 9.5 Feynman diagram builder, 11.5 Tensor calculator, 12.5 URL-shareable simulation state — each of which requires either a non-trivial new editor (9.5) or a coordinated touch of every existing sim (12.5), so they're best left for a focused follow-up.
